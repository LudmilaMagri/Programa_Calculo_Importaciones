...
/*
LinkedList* ll_filtrar3(LinkedList* pArrayListEmployee, int(*funcionCriterio)(void*))
{
	...
	if(pArrayListEmployee!=  NULL)
	{
		...
		for(i=0; i<len; i++)
		{
			...
			flag = 0;
			for(j=0; j<contadorHoras; j++)
			{
				horaEncontrada = funcionCriterioFiltrarHoras(pE);
				if (pE->horaEntrada == horaEncontrada)
				{
					flag = 1;
					break;
				}
...
{
	Empleado* pE;
	pE = (Empleado*) pGenerico;
	int horaAux;
	int flag;

	horaAux= empleado_getHoraEntrada(pE, &flag);
	if(horaAux != NULL)
	{
		return 0;
	}
